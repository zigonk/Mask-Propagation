...
if __name__ == "__main__":
    ...
    for vid in valid_video_list:
        video_info = meta_data['videos'][video_info]
        ...
        for eid in video_info['expressions'].keys():
            ...
            for ind in range(1, len(query_frames_id)):
                ...
                if (np.sum(np.asarray(previous_mask)) == 0):
                    ...
                    continue
                data = {}
                start_idx = full_frames_id.index(query_frames_id[ind - 1])
                end_idx = full_frames_id.index(query_frames_id[ind])
